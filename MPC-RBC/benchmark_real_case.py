...
def tuning_terminal_value(
    ...
    theta_candidates,
    hlimit_ft,
    # baseline trajectory (MPC-false H=96)
    traj_baseline,
    ...
    # baseline metrics (single-controller df)
    base_df = compute_metrics({"BASELINE": traj_baseline}, horizon=96, model=model)
    base = base_df.loc["BASELINE"]

    for H in horizons:
        for lam in lambdas:
            name = f"LAO-term(H={H},λ={lam})"
            traj = run_lao_terminal_value_mpc(
                model=model,
                x0=x0,
                u0=u0,
                qin_forecast=qin_f,
                ...
                hlimit_ft=hlimit_ft,
                ckpt_path=ckpt_path,
                terminal_weight=float(lam),
            )

            dt = traj.elapsed_time
            df = compute_metrics({name: traj}, horizon=H, model=model)
            r = df.loc[name]
            ...
            rows.append({
                ...
                # absolute metrics
                "Overflow_m3": r["Overflow_m3"],
                "PeakQ_m3s": r["PeakQ_m3s"],
                "Load": r["Load"],
                "Effort": r["Effort"],
                "Smooth": r["Smooth"],

                # relative to baseline (multiplicative ratio)
                "PeakQ_ratio": r["PeakQ_m3s"] / base["PeakQ_m3s"] if base["PeakQ_m3s"] > 0 else np.nan,
                "Load_ratio":  r["Load"]     / base["Load"]      if base["Load"] > 0 else np.nan,
                "Smooth_ratio":r["Smooth"]   / base["Smooth"]    if base["Smooth"] > 0 else np.nan,
                "Effort_ratio":r["Effort"]   / base["Effort"]    if base["Effort"] > 0 else np.nan,
            })

            print(f"[DONE] H={H}, λ={lam} | time={dt:.2f}s | "
                  f"PeakQ={r['PeakQ_m3s']:.4g} (x{rows[-1]['PeakQ_ratio']:.2f}) | "
                  f"Smooth={r['Smooth']:.4g} (x{rows[-1]['Smooth_ratio']:.2f}) | "
                  f"Load={r['Load']:.4g} (x{rows[-1]['Load_ratio']:.2f})")

    res = pd.DataFrame(rows)

    # (Smooth, PeakQ, Load) runtime
    res_sorted = res.sort_values(
        by=["Smooth_ratio", "PeakQ_ratio", "Load_ratio", "runtime_s"],
...
def save_trajs_npz(out_path: Path, trajs: dict, meta: dict) -> None:
    """
    Save trajs + meta into a single npz file.
    trajs: Dict[str, ControllerTraj]
    meta:  any small scalars/arrays you want (dt, hlimit, etc.)
    """
    out = {}
    # meta
    for k, v in meta.items():
        out[f"meta__{k}"] = np.array(v)

    # trajs
    for name, tr in trajs.items():
        key = name.replace(" ", "_").replace("-", "_")
        out[f"{key}__h_ft"] = np.asarray(tr.h_ft)
        out[f"{key}__c"] = np.asarray(tr.c)
        out[f"{key}__qout_cfs"] = np.asarray(tr.qout_cfs)
        out[f"{key}__theta"] = np.asarray(tr.theta)
        out[f"{key}__qspill_cfs"] = np.asarray(tr.qspill_cfs) if tr.qspill_cfs is not None else np.array([])

    np.savez_compressed(out_path, **out)
    print(f"[save] trajs saved to: {out_path}")
...
def mpc_benchmark() -> None:
    ...
    data1 = pd.read_excel(data_path)  # true
    data2 = pd.read_excel(data_path)  # forecasts
    ...
    # LAO-pruned MPC
    # ------------------------------------------------------------------
    print("\nLAO-pruned MPC simulation...")
    # t0 = time.perf_counter()
    traj_lao_pruned = run_lao_pruned_mpc(
        model=model,
        x0=x0,
        u0=u0,
        qin_forecast=qin_f,
    ...
    # print(f"LAO-pruned MPC done in {time.perf_counter() - t0:.2f}s")


    print("\nLAO-terminal-value MPC simulation (short horizon)...")
    t0 = time.perf_counter()
    traj_lao_term = run_lao_terminal_value_mpc(
        model=model,
        x0=x0,
        u0=u0,
        qin_forecast=qin_f,
    ...
    # tune_df.to_csv("tuning_terminal_results_real_case.csv", index=False)
    # print("\nSaved tuning table to tuning_terminal_results_real_case.csv")
    ...
    # ------------------------------------------------------------------
    print("\nRBC simulations...")
    q_desired_cfs = float(np.max(traj_mpc_ekf.qout_cfs))
    climit = 5.0

    traj_rbc_out = rbc_outflow(model, MD_t, x0, q_desired_cfs=q_desired_cfs, hlimit_ft=float(np.max(traj_mpc_ekf.h_ft)))
    traj_rbc_con = rbc_concentration(model, MD_t, x0, climit=climit, hlimit_ft=float(np.max(traj_mpc_ekf.h_ft)))
    traj_rbc_both = rbc_both(model, MD_t, x0, climit=climit, q_desired_cfs=q_desired_cfs, hlimit_ft=float(np.max(traj_mpc_ekf.h_ft)))
    ...
    save_trajs_npz(
        out_path=base_dir / "results_trajs_real_case.npz",
        trajs=trajs,
        meta={
            "dt": model.dt,
            "hlimit_ft": hlimit_ft,
    ...
    # Save
    # ------------------------------------------------------------------
    out_path = base_dir / "mpc_benchmark_results_real_case.mat"
