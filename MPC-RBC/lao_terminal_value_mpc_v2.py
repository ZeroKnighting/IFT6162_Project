"""
lao_terminal_value_mpc_v2.py

Learning-Augmented MPC using a learned terminal value function V(x_H), **batch-vectorized**
to avoid calling the value network P times per step.

Changes vs lao_terminal_value_mpc.py:
- load_value_terminal_fn(...) now returns V_raw_batch(X: (N,2)) -> (N,) in RAW J* space
- plan_theta_pair_grid_fast_with_cost(...) calls terminal_value_fn once on the whole (P,2) batch
...
try:
    ...
    raise ImportError(
        "PyTorch is required for lao_terminal_value_mpc_v2.py. Install with `pip install torch`."
    ) from e
...
def load_value_terminal_fn(
    ...
    """
    Loads ValueNet and returns V_raw_batch(X_np)->np.ndarray in RAW J* space.

    Input:
        X_np: (N,2) or (2,)  (reshaped to (N,2))
    Output:
        V: (N,)  (RAW J* units)
    """
    if device is None:
    ...
    x_mean = np.asarray(ckpt.get("x_mean", np.zeros((1, 2), dtype=float)), dtype=float).reshape(1, 2)
    x_std = np.asarray(ckpt.get("x_std", np.ones((1, 2), dtype=float)), dtype=float).reshape(1, 2)
    ...
    @torch.no_grad()
    def V_raw_batch(X_np: np.ndarray) -> np.ndarray:
        ...
        z = vnet(xt).detach().cpu().numpy().reshape(-1, 1)
...
def plan_theta_pair_grid_fast_with_cost(
    ...
    hlimit_ft: float,
    terminal_value_fn: Optional[Callable[[np.ndarray], np.ndarray]] = None,
    terminal_weight: float = 1.0,
) -> Tuple[float, float, float]:
    """
    Vectorized grid search for (theta0, theta1) with CH=2 move-blocking.
    terminal_value_fn must be BATCH: (P,2)->(P,)
    """
    ...
    if terminal_value_fn is not None:
        ...
        V = np.asarray(terminal_value_fn(XH), dtype=float).reshape(-1)
        if V.shape[0] != P:
            raise ValueError(f"terminal_value_fn must return shape (P,), got {V.shape}")
...
def run_lao_terminal_value_mpc(
    ...
    """
    Deterministic MPC (CH=2) + learned terminal value V(x_H), batch-vectorized.
    """
    V_term_batch, _ = load_value_terminal_fn(ckpt_path=ckpt_path, device=device)
    ...
    for k in range(Tsim - horizon - 1):
        ...
        th0, th1, _ = plan_theta_pair_grid_fast_with_cost(
            ...
            hlimit_ft=hlimit_ft,
            terminal_value_fn=V_term_batch,   # batch fn
    ...
    print(f"LAO-terminal-value MPC (batch) done in {elapsed_time:.2f}s")
